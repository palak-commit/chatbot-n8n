const Sidebar = ({ activeTab, setActiveTab, handleLogout, isDarkMode, toggleDarkMode, isOpen = false, onClose = () => {} }) => {
  const handleNav = (tab) => {
    setActiveTab(tab);
    onClose();
  };
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-slate-900 border-r border-gray-100 dark:border-slate-800 flex flex-col transition-transform duration-300 md:static md:translate-x-0 ${
      ...
      >
        <div className="p-6 border-b border-gray-50 dark:border-slate-800 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="md:hidden rounded-xl p-2 text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800"
